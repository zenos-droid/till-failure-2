...
export default function PortalLayout() {
  ...
  // Fallback guard just in case direct URL hit bypasses route protective layers
  if (!user) {
    return (
  ...
  const handleProfileSave = (e: React.FormEvent) => {
    e.preventDefault();
    store.updateProfile(profileName, profilePhone);
  ...
  const roleMatchingNotices = store.notices.filter(n => {
    if (user.role === 'MEMBER') return n.targetRole === 'ALL' || n.targetRole === 'MEMBERS';
    if (user.role === 'TRAINER') return n.targetRole === 'ALL' || n.targetRole === 'TRAINERS';
    ...
    <div className="min-h-screen bg-zinc-950 text-white flex flex-col lg:flex-row relative">
      ...
      <aside className="hidden lg:flex w-72 bg-zinc-950 border-r border-zinc-900 flex-col justify-between shrink-0 sticky top-0 h-screen z-10">
        <div className="flex flex-col flex-1">
          ...
          <div className="p-6 border-b border-zinc-900 bg-zinc-900/20 space-y-3">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <button
                ...
                className="text-zinc-500 hover:text-[#FFE259] transition-colors p-1"
          ...
          {/* Sidebar functional navigation menus */}
      ...
      <nav className="lg:hidden sticky top-0 z-40 bg-zinc-950 border-b border-zinc-900 px-4 py-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 border border-zinc-850 rounded-lg text-zinc-400 hover:text-white"
          >
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="space-y-1">
              <span className="block text-xs font-sans font-bold text-white uppercase">{user.name}</span>
              <span className="block text-[10px] font-mono text-zinc-500">{user.email}</span>
            </div>
