...
for (const m of members) {
    await prisma.member.create({ data: { ...m, startDate: new Date(m.startDate), endDate: new Date(m.endDate) } });
...
for (const p of payments) {
    await prisma.paymentRecord.create({ data: { ...p, paymentDate: new Date(p.paymentDate), expiryExtendedDate: new Date(p.expiryExtendedDate) } });
