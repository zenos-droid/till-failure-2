...
function signAccess(user) {
    return jwt.sign({ id: user.id, role: user.role, email: user.email }, env.JWT_SECRET, { expiresIn: env.ACCESS_TOKEN_TTL });
}
function signRefresh(user) {
    return jwt.sign({ id: user.id, role: user.role, email: user.email, type: 'refresh' }, env.REFRESH_TOKEN_SECRET, { expiresIn: env.REFRESH_TOKEN_TTL });
}
export async function login(email, password, role, ip) {
    const user = await prisma.user.findUnique({ where: { email } });
    if (!user || user.role !== role || !user.isActive)
        throw new AppError(401, 'Invalid credentials or role');
...
export async function logout(userId, refreshToken, ip) {
    if (refreshToken) {
...
export async function updateProfile(userId, name, phone) {
    ...
    if (user.memberId)
        await prisma.member.update({ where: { id: user.memberId }, data: { name, phone } });
    if (user.trainerId)
        await prisma.trainer.update({ where: { id: user.trainerId }, data: { name, phone } });
    if (user.receptionistId)
        await prisma.receptionist.update({ where: { id: user.receptionistId }, data: { name, phone } });
